...
export const LOCAL_STORE_KEYS = [
  ...
  "aiProviderConfig", // §8: konfigurasi model AI pemilik (kompatibel OpenAI) — lokal di perangkat
] as const;
...
export interface LocalStore {
  ...
  // ---------------------------------------- Model AI pemilik (§8, opsional)
  // Null = mode lokal (jawaban deterministik, offline). Kunci API TIDAK
  // pernah dikirim ke Google Sheets — hanya dipakai langsung oleh provider.
  getAiProviderConfig(): Promise<import("@/services/ai/ai-provider").RemoteAiConfig | null>;
  setAiProviderConfig(config: import("@/services/ai/ai-provider").RemoteAiConfig | null): Promise<void>;
