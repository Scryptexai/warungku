...
## Status: PHASE 3 — Core Transaction & Barcode Workflow ✅

| Area | Status |
| --- | --- |
| Kerangka aplikasi + navigasi bawah + Beranda gaya dompet digital | ✅ Tahap 1–2 |
| Produk: daftar, cari (nama/barcode), tambah, edit, detail | ✅ Tahap 2 |
...
| Tab | Kolom |
| --- | --- |
| `PRODUCTS` | product_id, barcode, product_name, category, price, stock, unit, updated_at |
| `TRANSACTIONS` | transaction_id, date, time, payment_type, customer_name, total_amount |
| `TRANSACTION_ITEMS` | transaction_id, barcode, product_name, quantity, unit_price, subtotal |
...
3. **PHASE 3 — Core transaction & barcode workflow** ✅ *(saat ini)*
4. PHASE 4 — Search, customer/bon & reporting
...
## 10. Lanjut ke Tahap 4

Tahap 4 (pencarian, customer/bon & laporan) tinggal MEMBACA spreadsheet
warung lewat jalur yang sudah ada (`HttpGoogleApiClient` → proksi):
riwayat transaksi di tab `TRANSACTIONS`/`TRANSACTION_ITEMS`, daftar bon di
`CUSTOMERS`, laporan harian/mingguan/bulanan dari kolom date/total — tanpa
