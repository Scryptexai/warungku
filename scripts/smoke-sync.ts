...
import { AssistantService } from "../src/services/ai/assistant.service";
import { RemoteLlmProvider } from "../src/services/ai/remote-llm-provider";
import type { AiAnswer, AiExplainInput, AiProvider } from "../src/services/ai/ai-provider";
import type { Customer } from "../src/domain";
import {
...
async function main(): Promise<void> {
  ...
  // ================================================== TAHAP 8
  console.log("10) UJI PENERIMAAN TAHAP 8 — ASISTEN AI TOKO (DATA LOKAL)");
  const store8 = new MemoryLocalStore();
  const now8 = new Date("2026-09-01T12:00:00+07:00"); // Selasa, WIB

  // Katalog: 5 produk nyata + 25 pengisi (semua tanpa harga modal).
  const mkProduct8 = (id: string, name: string, price: number, stock: number, barcode: string): Product => ({
    id, barcode, name, currentPrice: price, costPrice: null, stock,
    unit: "pcs", category: "Uji", isActive: true,
    createdAt: "2026-08-01T00:00:00.000Z", updatedAt: "2026-08-01T00:00:00.000Z",
  });
  const products8: Product[] = [
    mkProduct8("prd_aqua", "Aqua 600ml", 3000, 8, "8886008101053"),
    mkProduct8("prd_kopi", "Kopi X", 2000, 5, gs1Barcode("899111000001")),
    mkProduct8("prd_mie", "Mie Y", 3000, 6, gs1Barcode("899111000002")),
    mkProduct8("prd_gula", "Gula Pasir 1kg", 10000, 100, gs1Barcode("899111000003")),
    mkProduct8("prd_rokok", "Rokok A", 20000, 50, gs1Barcode("899111000004")),
  ];
  for (let i = 0; i < 25; i += 1) {
    products8.push(mkProduct8(`prd_isi_${i}`, `Produk Isi ${i}`, 1000, 10, gs1Barcode(`89911101${String(i).padStart(4, "0")}`)));
  }
  await store8.setCachedProducts(products8);

  const mkTrx8 = (args: {
    id: string; ts: string; pay: "CASH" | "BON"; cust?: { id: string; name: string };
    items: Array<[Product, number]>;
  }): Transaction => {
    const items = args.items.map(([product, qty]) => ({
      transactionId: args.id, productId: product.id, barcode: product.barcode,
      productName: product.name, quantity: qty, unitPrice: product.currentPrice,
      subtotal: qty * product.currentPrice,
    }));
    return {
      id: args.id, timestamp: args.ts, customer: args.cust ?? null,
      paymentType: args.pay, total: items.reduce((sum, item) => sum + item.subtotal, 0),
      status: "COMPLETED", paymentStatus: args.pay === "BON" ? "UNPAID" : "PAID",
      items, note: null, syncedAt: null,
    };
  };
  const trxs8: Transaction[] = [
    // Minggu lalu (24–25 Agu — jumlah hari sama dgn minggu ini berjalan).
    mkTrx8({ id: "trx8_p1", ts: "2026-08-25T09:00:00+07:00", pay: "CASH", items: [[products8[3]!, 5]] }),      // 50.000
    mkTrx8({ id: "trx8_p2", ts: "2026-08-24T10:00:00+07:00", pay: "BON", cust: { id: "cst_siti", name: "Siti" }, items: [[products8[1]!, 75]] }), // 150.000
    mkTrx8({ id: "trx8_p3", ts: "2026-08-25T11:00:00+07:00", pay: "CASH", items: [[products8[3]!, 5]] }),      // 50.000 → lalu 250.000
    mkTrx8({ id: "trx8_p4", ts: "2026-08-26T08:00:00+07:00", pay: "BON", cust: { id: "cst_andi", name: "Andi" }, items: [[products8[3]!, 25]] }),  // 250.000 (bon lama)
    mkTrx8({ id: "trx8_p5", ts: "2026-08-26T09:00:00+07:00", pay: "BON", cust: { id: "cst_budi", name: "Budi" }, items: [[products8[4]!, 41]] }),   // 820.000 (bon lama)
    // Minggu ini (Senin 31 Agu — kemarin).
    mkTrx8({ id: "trx8_w1", ts: "2026-08-31T09:00:00+07:00", pay: "CASH", items: [[products8[0]!, 50]] }),     // 150.000
    mkTrx8({ id: "trx8_w2", ts: "2026-08-31T10:00:00+07:00", pay: "CASH", items: [[products8[1]!, 30]] }),     // 60.000
    // Hari ini (Selasa 1 Sep).
    mkTrx8({ id: "trx8_t1", ts: "2026-09-01T08:00:00+07:00", pay: "CASH", items: [[products8[0]!, 13], [products8[2]!, 3]] }), // 48.000
    mkTrx8({ id: "trx8_t2", ts: "2026-09-01T09:00:00+07:00", pay: "CASH", items: [[products8[1]!, 1]] }),      // 2.000
    mkTrx8({ id: "trx8_t3", ts: "2026-09-01T10:00:00+07:00", pay: "BON", cust: { id: "cst_budi", name: "Budi" }, items: [[products8[2]!, 10]] }), // 30.000
  ];
  for (const trx of trxs8) await store8.upsertTransaction(trx);

  const mkCustomer8 = (id: string, name: string, balance: number): Customer => ({
    id, name, phone: null, address: null, outstandingBalance: balance,
    creditLimit: null, isActive: true,
    createdAt: "2026-08-20T00:00:00.000Z", updatedAt: "2026-08-20T00:00:00.000Z",
  });
  await store8.setCachedCustomers([
    mkCustomer8("cst_budi", "Budi", 850000),
    mkCustomer8("cst_andi", "Andi", 250000),
    mkCustomer8("cst_siti", "Siti", 150000),
  ]);

  const assistant8 = new AssistantService({ localStore: store8 });

  // TEST 1 — omzet hari ini = laporan deterministik (80.000; 3 transaksi).
  const a1 = await assistant8.ask("Omzet hari ini berapa?", now8);
  check(
    a1.text.includes(formatIDR(80000)) && a1.text.includes("3 transaksi") &&
      a1.sourceLabel.includes("WIB"),
    "8.T1 OMZET: jawaban = laporan deterministik (Rp80.000, 3 transaksi) + sumber berperiode",
  );

  // TEST 2 — produk terlaris minggu ini = kueri produk (Aqua 63).
  const a2 = await assistant8.ask("Produk paling laku minggu ini?", now8);
  check(
    a2.text.includes("Aqua 600ml") && a2.text.includes("63") && a2.mode === "lokal",
    "8.T2 TERLARIS: Aqua 600ml — 63 terjual (mode lokal, offline)",
  );

  // TEST 3 — bon aktif = data bon (3 pelanggan, 1.250.000, terbesar Budi).
  const a3 = await assistant8.ask("Siapa yang masih punya Bon?", now8);
  check(
    a3.text.includes("3 pelanggan") && a3.text.includes(formatIDR(1250000)) &&
      a3.text.includes("Budi") && a3.text.includes(formatIDR(850000)),
  ...
  check(
    a4.text.includes("naik") && a4.text.includes(formatIDR(40000)) && a4.text.includes("16%"),
    "8.T4 PERBANDINGAN: 290.000 vs 250.000 → naik Rp40.000 / 16% (dihitung aplikasi)",
  );

  // TEST 5 — keuntungan: harga modal tidak tersedia → tolak jujur.
  const a5 = await assistant8.ask("Berapa keuntungan saya bulan ini?", now8);
  check(
    /belum bisa menghitung keuntungan/i.test(a5.text) && !a5.text.includes("untung Rp"),
    "8.T5 KEUNTUNGAN: ditolak jujur — harga modal belum tersedia (bukan angka karangan)",
  );

  // TEST 8 — halusinasi: di luar data toko & produk tak dikenal.
  const a8a = await assistant8.ask("Siapa presiden Indonesia sekarang?", now8);
  check(!/Rp\d/.test(a8a.text) && /data toko/i.test(a8a.text), "8.T8 HALUSINASI: di luar data toko → ditolak tanpa angka karangan");
  const a8b = await assistant8.ask("Stok Sabun Ajaib berapa?", now8);
  check(/tidak menemukan produk/i.test(a8b.text) && !/stok \d/.test(a8b.text.split(".")[0] ?? ""), "8.T8 HALUSINASI: produk tak dikenal → dijawab tidak ditemukan");

  // Watchlist stok: aturan deterministik stok < terjual 7 hari.
  const aStok = await assistant8.ask("Stok apa yang mulai sedikit?", now8);
  check(
    aStok.text.includes("perlu diperhatikan") && aStok.text.includes("Aqua 600ml") && aStok.text.includes("63"),
    "8.STOK: watchlist deterministik (Aqua stok 8 < terjual 63)",
  );

  // TEST 7 — penyedia AI gagal → jawaban lokal tetap keluar, tanpa crash.
  const exploding = new (class implements AiProvider {
    id = "exploding";
    async explain(): Promise<AiAnswer> {
      throw new Error("BOOM — model tidak tersedia");
    }
  })();
  const assistantFallback = new AssistantService({
    localStore: store8,
    providerFactory: () => exploding,
  });
  const a7 = await assistantFallback.ask("Omzet hari ini berapa?", now8);
  check(
    a7.usedFallback && a7.mode === "lokal" && a7.text.includes(formatIDR(80000)),
    "8.T7 AI MATI: model gagal → jawaban deterministik tetap keluar (tanpa crash)",
  );

  // TEST 6 — kasir offline penuh + asisten membaca hasilnya.
  const store6c = new MemoryLocalStore();
  const repo6c = new GoogleSheetsStoreRepository(new NotConnectedGoogleApiClient());
  const engine6c = new QueueSyncEngine({ target: new NotConnectedSyncTarget(), localStore: store6c, listenToNetworkEvents: false });
  await engine6c.init();
  const products6c = new ProductService({ repository: repo6c, localStore: store6c, syncEngine: engine6c });
  const customers6c = new CustomerService({ repository: repo6c, localStore: store6c, syncEngine: engine6c });
  const transactions6c = new TransactionService({ localStore: store6c, syncEngine: engine6c, repository: repo6c });
  const sales6c = new SaleService(products6c, transactions6c, customers6c, engine6c);
  const pOffline = await products6c.createProduct({ name: "Aqua 600ml", barcode: "8886008101053", category: "Minuman", currentPrice: 3000, stock: 10, unit: "pcs" });
  const saleOffline = await sales6c.recordSale({ items: [{ productId: pOffline.id, quantity: 2 }], paymentType: "CASH" });
  const assistantOffline = new AssistantService({ localStore: store6c });
  const a6 = await assistantOffline.ask("Omzet hari ini berapa?");
  check(
    saleOffline.transaction.total === 6000 && a6.text.includes(formatIDR(6000)),
    "8.T6 OFFLINE: kasir tetap jalan (jual 2×3.000) & asisten membacanya tanpa internet",
  );

  // TEST 10 — privasi: hanya fakta minimal yang dikirim ke penyedia.
  const captured: AiExplainInput[] = [];
  const spy = new (class implements AiProvider {
    id = "spy";
    async explain(input: AiExplainInput): Promise<AiAnswer> {
      captured.push(input);
      return { text: "jawaban model", mode: "model" };
    }
  })();
  const assistantSpy = new AssistantService({ localStore: store8, providerFactory: () => spy });
  const a10 = await assistantSpy.ask("Omzet minggu ini?", now8);
  const payload10 = JSON.stringify(captured[0]!.facts);
  check(
    a10.mode === "model" && payload10.length < 1200 && !payload10.includes("trx8_") &&
      !payload10.includes("items") && captured[0]!.facts.omzet === 290000,
    "8.T10 PRIVASI: model hanya menerima fakta ringkas (tanpa transaksi mentah)",
  );

  // Adapter jarak jauh: bentuk permintaan benar + gagal → fallback.
  let remoteUrl = "";
  let remoteBody = "";
  const remoteOk = new RemoteLlmProvider(
    { baseUrl: "https://api.contoh.ai/v1", apiKey: "kunci-uji", model: "model-uji" },
    (async (url: string | URL, init?: RequestInit) => {
      remoteUrl = String(url);
      remoteBody = String(init?.body);
      return new Response(JSON.stringify({ choices: [{ message: { content: "Jawaban dari model." } }] }), { status: 200 });
    }) as unknown as typeof fetch,
  );
  const remoteAnswer = await remoteOk.explain({
    question: "Omzet minggu ini?", intent: "SALES",
    facts: captured[0]!.facts, sourceLabel: "uji", deterministicAnswer: "uji",
  });
  check(
    remoteAnswer.mode === "model" && remoteAnswer.text === "Jawaban dari model." &&
      remoteUrl.endsWith("/chat/completions") && remoteBody.includes("fakta") &&
      !remoteBody.includes("trx8_"),
    "8.ADAPTER: kompatibel OpenAI, kirim Bearer + fakta minimal saja",
  );
  const remoteDead = new RemoteLlmProvider(
    { baseUrl: "https://api.contoh.ai/v1", apiKey: "kunci-uji", model: "model-uji" },
    (async () => {
      throw new Error("network down");
    }) as unknown as typeof fetch,
  );
  const assistantRemoteDead = new AssistantService({ localStore: store8, providerFactory: () => remoteDead });
  const aRemoteDead = await assistantRemoteDead.ask("Omzet hari ini berapa?", now8);
  check(
    aRemoteDead.usedFallback && aRemoteDead.text.includes(formatIDR(80000)),
    "8.ADAPTER: model tak terjangkau → fallback lokal otomatis (kasir aman)",
  );

  // TEST 9 — dataset besar: 720 produk / 520 transaksi / 2.080 item.
  const storeBig = new MemoryLocalStore();
  const productsBig: Product[] = Array.from({ length: 720 }, (_, i) =>
    mkProduct8(`prd_perf_${i}`, `Produk Uji ${i}`, 1500, i < 5 ? 3 : 100, gs1Barcode(`89977${String(i).padStart(7, "0")}`)),
  );
  await storeBig.setCachedProducts(productsBig);
  const trxsBig: Transaction[] = Array.from({ length: 520 }, (_, i) => {
    const items = [0, 1, 2, 3].map((k) => {
      const product = productsBig[(i * 4 + k) % 720]!;
      return {
        transactionId: `trx_perf_${i}`, productId: product.id, barcode: product.barcode,
        productName: product.name, quantity: 2, unitPrice: 1500, subtotal: 3000,
      };
    });
    return {
      id: `trx_perf_${i}`,
      timestamp: `2026-08-${String(3 + (i % 28)).padStart(2, "0")}T1${i % 8}:00:00+07:00`,
      customer: null, paymentType: i % 4 === 0 ? "BON" : "CASH", total: 12000,
      status: "COMPLETED" as const,
      paymentStatus: i % 4 === 0 ? ("UNPAID" as const) : ("PAID" as const),
      items, note: null, syncedAt: null,
    };
  });
  for (const trx of trxsBig) await storeBig.upsertTransaction(trx);
  const assistantBig = new AssistantService({ localStore: storeBig });
  const nowBig = new Date("2026-08-31T12:00:00+07:00");
  const t9Start = Date.now();
  const q9a = await assistantBig.ask("Omzet bulan ini berapa?", nowBig);
  const q9b = await assistantBig.ask("Produk paling laku bulan ini?", nowBig);
  await assistantBig.ask("Stok apa yang mulai sedikit?", nowBig);
  const t9Ms = Date.now() - t9Start;
  check(
    q9a.text.includes(formatIDR(6240000)) && q9b.text.includes("Produk Uji") && t9Ms < 1500,
    `8.T9 DATA BESAR: 3 jawaban akurat dalam ${t9Ms} ms (720 produk / 520 transaksi)`,
  );
  ...
  console.log("\nSemua pemeriksaan lolos — Tahap 3 + 5A + 5B + 5D + 6 + 7 + 8 (asisten AI atas data lokal) berfungsi.");
